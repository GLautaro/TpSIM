...
class Cliente:
    ...
    def completarColumnas(self, df, nombre_col_estado, nombre_col_maquina, row):
        if nombre_col_estado in df.columns:
            ...
            df.at[row, nombre_col_maquina] = self.maquina.id_maquina if self.maquina is not None else None
        else:
            row = len(df) - 1
            nones = ["" for i in range(row)]
            df[nombre_col_estado] = nones + [self.estado]
            df[nombre_col_maquina] = nones + [self.maquina.id_maquina if self.maquina is not None else None]
        return df
